...
async function happyPath(page: Page, missionId: string, request: APIRequestContext) {
  ...
  await expect(page.getByTestId("panel-mission-console")).toContainText("runtime-demo-pact");
  ...
  await expect(challengePanel).toContainText("runtime_fallback_required");
  ...
  expect(dryRunPayload.settlementPath).toBe("runtime_fallback_required");
  ...
  await expect(page.getByTestId("panel-caw-execution-timeline")).toContainText("Runtime guard reached CAW boundary");
  await expect(page.getByTestId("panel-official-caw-panel")).toContainText("fallback_required");
  ...
  await expect(page.getByTestId("panel-service-receipt-panel")).toContainText("n/a");
  await expect(page.getByTestId("panel-official-caw-panel")).toContainText("fallback");
